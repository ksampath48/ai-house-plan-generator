export function generatePlan(input = {}) {
  const plotSize = clampNumber(input.plotSize, 20, 5000, 200);
  const floors = clampNumber(input.floors, 1, 4, 1);
  const bedrooms = clampNumber(input.bedrooms, 1, 6, 3);
  const bathrooms = clampNumber(input.bathrooms, 1, 5, 2);
  ...
  const roomMix = [
    {
      name: "Living Room",
      area: Math.round(floorPlate * 0.2),
      floor: 1,
      position: "Front-center",
      notes: "Double height optional for premium layouts",
    },
    {
      name: "Dining",
      ...
      notes: "Direct visual connection to living",
    },
    {
      name: "Kitchen",
      area: Math.round(floorPlate * 0.1),
      floor: 1,
      position: input.vastu === "Yes" ? "South-East" : "Adjacent to dining",
      notes: `${input.kitchenType || "Open"} kitchen`,
    },
    {
      name: "Guest Bedroom",
      area: Math.round(floorPlate * 0.11),
      floor: 1,
      position: "Ground floor",
      notes: "Accessible washroom nearby",
    },
  ];
  ...
  if (input.parking === "Yes") {
    roomMix.push({
      name: "Parking",
      area: Math.round(sqft * 0.12),
      floor: 1,
      position: "Front setback",
      notes: "2-car stack possible",
    });
  }
